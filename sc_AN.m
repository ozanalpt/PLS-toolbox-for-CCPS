function [security_scAN,QoS_scAN] = sc_AN(x_array,y_array,loc_weight,Pos_a,Pos_b,Pos_j,P_s,P_w,P_j,sim)
%% Subcarrier based AN
...
SINR_b = (abs(h_ab)^2*P_s_lin)/(2*No+abs(h_jb)^2*Pj);
SINR_e = (abs(h_ae)^2*P_s_lin)/(abs(h_ae)^2*Pw+No+abs(h_je)^2*Pj);
C_s = max([0,(log2(1+SINR_b)-log2(1+SINR_e))]);
CC = CC+C_s;
Qos = Qos+SINR_b;
end
QoS_scAN(cnt_3) = Qos/sim;
